...
from datalayer_utils import now_string
...
def user_exists(username):
    return len(get_users_by_username(username)) > 0


def get_user_by_username(username):
    users = __get_solr().search(f'id:{username}', rows=50)
...
def get_users_by_username(username):
    users = __get_solr().search(f'id:{username}', rows=50)
    logger.debug(f'Found {len(users)} user(s) for username {username}')
    for user in users:
...
def get_users_by_email(email):
    users = __get_solr().search(f'email_s:{email}', rows=50)
...
def delete_users(q):
    CONFIRM = f'I CONFIRM I WANT TO PERMANENTLY DELETE USERS {q}'
