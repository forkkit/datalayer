...
class DspCli(object):
    ...
    def iam_parser(self):
        ...
        parser_iam.add_argument('-q', '--query', default='*:*', help='Query to be applied to the given command)')
        parser_iam.add_argument('-f', '--filter', help='Filter to apply on uid (check if uid contains the given value)')
        parser_iam.add_argument('-d', '--details', action='store_true', help='Print details')


    def iam(self, args):
        from .iam.iam import execute_iam
        execute_iam(args)
