...
# --- Common ---
c.JupyterHub.base_url = '/api/jupyterhub'
...
#    c.Spawner.cmd = [ os.path.join(os.environ['DLAHOME'], 'src', 'dev', 'conf', 'jupyterhub', 'launch-lab.sh') ]
...
def docker_spawner(c):
    ...
    c.Spawner.environment = { 'DLA_GITHUB_TOKEN': os.environ['DLA_GITHUB_TOKEN']}
    from jupyter_client.localinterfaces import public_ips
    c.JupyterHub.hub_ip = public_ips()[0]
